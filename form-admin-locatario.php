<?php 
  ...
  if ( !empty($_GET['codigo_locatario'])) {
    $codigo_locatario = $_REQUEST['codigo_locatario'];
  }
  
  if ( null == $codigo_locatario ) {
    header("Location:form-main-locatario.php");
  }
  ...
  if ( !empty($_POST)) {
    ...
    // insert data
    if ($valid) {
        $sql = "UPDATE locatario SET nome_locatario='$nome', estado_locatario='$uf', profissao_locatario='$profissao', 
        rg_locatario='$rg', cpf_locatario='$cpf', hash_locatario='$hash' WHERE codigo_locatario= $codigo_locatario";
        $resultado = mysqli_query($link, $sql);
        
       if($resultado){
          echo "<script>location='form-main-locatario.php'</script>";
       }else{
          echo "<script>location='form-admin-locatario.php?codigo_locatario=$codigo_locatario'</script>";
       }
    }
  
  } else {
      $sql = "SELECT * FROM locatario WHERE codigo_locatario= $codigo_locatario"; 
      $resultado = mysqli_query($link, $sql);
      $registro = mysqli_fetch_array($resultado);
      $id       = $registro['codigo_locatario'];
      $nome     = $registro['nome_locatario'];
      $cpf      = $registro['cpf_locatario'];
      $rg       = $registro['rg_locatario'];
      $uf       = $registro['estado_locatario'];
      $profissao = $registro['profissao_locatario'];
      $hash     = $registro['hash_locatario'];
...
<nav id="myNavbar" class="navbar navbar-default navbar-inverse navbar-fixed-top" role="navigation">
  ...
  <div class="container-fluid">
      <div class="col-xs-12 col-md-6 col-sm-6 col-centered">
        <div class="row">
          <h3>Atualizar Locatario</h3>
        </div>
        <form class="form" action="form-admin-locatario.php?codigo_locatario=<?php echo $codigo_locatario?>" method="post">
